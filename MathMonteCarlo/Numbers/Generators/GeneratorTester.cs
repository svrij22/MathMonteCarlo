...
namespace MathMonteCarlo.Numbers
{
    ...
    internal class GeneratorTester
    {
        ...
        {
            Test<CongruentialNumberGenerator>(new CongruentialNumberGenerator());
        }
        ...
        public static void Test<T>(NumberGenerator numberGenerator)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                var Freq = Frequency(results);
                var Sobs = Math.Abs(Freq) / Math.Pow(amountOfNumbers, 0.5);
                var pValue = Erfc(Sobs * Math.Pow(2, 0.5)); 

                MCViewModel.Log($"Number Generator Tests T={typeof(T)}", $"Freq : {Freq}");
                MCViewModel.Log($"Number Generator Tests T={typeof(T)}", $"Sobs : {Sobs}");
                MCViewModel.Log($"Number Generator Tests T={typeof(T)}", $"P-value : {pValue}");
